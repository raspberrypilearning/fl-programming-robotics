...
Now that you've attached the ultrasonic distance sensor (UDS), you need to test that it can detect objects at different distances directly in front of it.

### Testing the UDS

Firstly, you need to check the UDS is working correctly whilst the robot is stationary. To interact with the UDS, you will be using the GPIO Zero library.
...
The code above works by repeatedly replacing the value of `pulse_start` with the current time until the **Echo** pin is active. This means that the final value of `pulse_start` is the time at which the **Echo** pin is set to high.
...
Try typing the above line of code when your hand is different distances in front of the UDS. You should get smaller values as your hand approaches the sensor.
...
The speed of sound in air will vary depending on conditions, but it tends to hover around 343 metres per second.
...
Using a different function to calculate this distance is good coding practice as this function can be tested separately, and easily reused multiple times within the program.
